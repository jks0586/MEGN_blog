...
const index = () => {
  ...
  const handleEdit=(e)=>{
    ...
    if(actionnode.dataset.id!==undefined){
      router.push(routeslink.category.edit.replace(":id",actionnode.dataset.id));
    }
  }

  const handleCreate=(e)=>{
    router.push(adminlink.category.create);
  }
  const handleDelete= async (e)=>{
    ...
    if(actionnode.dataset.id!==undefined){
    laxios
          .delete(routeslink.category.delete.replace(":id",actionnode.dataset.id))
          .then((response) => {
            if(response.status==200){
              swal("Congratulations!", "Category has been removed successfully", "success").then((value) => {
  ...
  const [columnDefs] = useState([
    ...
    { headerName:'Image',field: 'image',cellRenderer: params => {
      return  <img src={routeslink.imageurl+params.value} alt="me" width="64" height="64" />
    }  },
